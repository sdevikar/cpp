...
// the lamda function is a class of type lambda_<id>, where id is some autogenerated number
std::cout << typedid(lambda_function) << std::endl;
```
...
// passing lanbda expression as a comparator
...
- In order for the lambda expression to see the variable in the capture list, *it must be declared before the lambda expression*
